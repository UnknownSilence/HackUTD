<template>
  <v-app id="inspire">
    <v-navigation-drawer v-model="drawer" app>
      <v-list dense>
        <v-list-item @click="$router.push('')">
          <v-list-item-action>
            <v-icon>mdi-shield-airplane-outline</v-icon>
          </v-list-item-action>
          <v-list-item-content>
            <v-list-item-title>Home</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list>
    </v-navigation-drawer>

    <v-app-bar app color="#5FC9F4" append="dark">
      <v-app-bar-nav-icon @click.stop="drawer = !drawer"></v-app-bar-nav-icon>
      <v-toolbar-title>{{ appName }}</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-icon right>mdi-shield-airplane-outline</v-icon>
    </v-app-bar>
    <v-content>
      <v-container fill-height grid-list-md text-xs-center>
        <v-layout row wrap>
          <v-flex fill-height height="100%;">
            <v-card
              fill-height
              justify
              class="my-1"
              style="text-color: #5FC9F4;"
              color="#FFFFFF"
              height="100%;"
            >
              <v-card-subtitle
                color="#5FC9F4"
                style="text-color: #5FC9F4;"
                text-color="#5FC9F4; color: #5FC9F4;"
                class="center"
              >You are:</v-card-subtitle>
              <v-card-text style="color: #5FC9F4;">{{ userStatus }}</v-card-text>
            </v-card>
            <v-card
              fill-height
              justify
              class="my-1"
              style="text-color: #5FC9F4;"
              color="#FFFFFF"
              height="100%;"
            >
              <v-card-title
                color="#5FC9F4"
                style="text-color: #5FC9F4;"
                text-color="#5FC9F4; color: #5FC9F4;"
                class="center"
              >Lets Fly!</v-card-title>
              <v-btn
                @click="$router.push('booking')"
                append="icon"
                class="ma-1 pa-5"
                :style="{left: '50%', transform:'translateX(-50%)'}"
                color="#5FC9F4"
                min-width="70%"
                outlined
              >
                Book a Flight
                <v-icon right>mdi-airplane-takeoff</v-icon>
              </v-btn>
              <v-btn
                @click="$router.push('status')"
                class="ma-1 pa-5"
                :style="{left: '50%', transform:'translateX(-50%)'}"
                min-width="70%"
                color="#5FC9F4"
                outlined
              >
                Check Status
                <v-icon right>mdi-account-check-outline</v-icon>
              </v-btn>
              <br />
              <br />
              <v-img src="../assets/illustration-big.png"></v-img>
            </v-card>
          </v-flex>
        </v-layout>
      </v-container>
    </v-content>

    <v-footer color="#5FC9F4" app>
      <span>&copy; {{ eventName + " " + year }}</span>
    </v-footer>
  </v-app>
</template>

    <script>
export default {
  name: "Home",
  components: {},
  icons: {
    iconfont: "md"
  },
  data: () => ({
    drawer: null,
    alignment: "center",
    year: 2019,
    eventName: "HackUTD",
    appName: "Journey",
    dense: true,
    userStatus: "A frequent Flyer!"
  })
};
</script>